...
if(isset($_POST['submit_btn'])){
  ...
  $end_date = $_POST['end'];
  $read_query = "SELECT  `userid`, `fname`, `date`, `punchin`, `punchout`,`lunchin`,`lunchout`, `upwork_task`,`webtracker_task`, `webtracker`, concat(timestampdiff(HOUR,`punchin`,`punchout`),':', timestampdiff(MINUTE,`punchin`,`punchout`)%60,':',
  timestampdiff(second,`punchin`, `punchout`)%60 ) as 'Worked_Time' FROM todowork where  `date`>= '$start_date' AND `date` <= '$end_date'";
  $data = mysqli_query($myConnection, $read_query);

}
else{
  $read_query = "SELECT  `userid`, `fname`, `date`, `punchin`, `punchout`,`lunchin`,`lunchout`, `upwork_task`,`webtracker_task`,`webtracker`, concat(timestampdiff(HOUR,`punchin`,`punchout`),':', timestampdiff(MINUTE,`punchin`,`punchout`)%60,':',
  timestampdiff(second,`punchin`, `punchout`)%60 ) as 'Worked_Time' FROM todowork ";
  $data = mysqli_query($myConnection, $read_query);
...
if(mysqli_num_rows($data)>0){
  while($row = mysqli_fetch_array($data)){
    ...
    echo "<td style='text-align:center;'>".$row['webtracker']."</td>";
